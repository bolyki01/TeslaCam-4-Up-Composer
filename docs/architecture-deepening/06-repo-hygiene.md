...
6. Add CI follow-up plan after local lanes are stable. **Done.** `.github/workflows/python-tests.yml` covers the Python lanes on Ubuntu (Python 3.10 + 3.12, ffmpeg installed). `.github/workflows/native-tests.yml` runs `TeslaCamTests` on `macos-26` (Xcode 26.5 beta, macOS 26 SDK) so the project's `MACOSX_DEPLOYMENT_TARGET=26.0` builds cleanly without overrides. Code signing is disabled for the CI build; the bookmark round-trip test runs in plain (non-security-scoped) mode via injected options on `SourceStore`. UI tests stay out of CI — slow + flaky; `script/test_native.sh` is the local lane.
